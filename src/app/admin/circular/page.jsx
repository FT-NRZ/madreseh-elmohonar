...
export default function CircularAdminPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.content.trim()) {
      alert('محتوای بخشنامه الزامی است');
      return;
    }
    ...
    const submitData = {
      ...
      target_teacher_id: form.target_type === 'specific_teacher' ? parseInt(form.target_teacher_id) : null,
    };
    ...
    try {
      ...
      if (data.success) {
        resetForm();
        fetchCirculars();
        alert(form.id ? 'بخشنامه با موفقیت ویرایش شد' : 'بخشنامه با موفقیت ثبت شد');
      } else {
